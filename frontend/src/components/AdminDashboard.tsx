...
const AdminDashboard: React.FC = () => {
  ...
  const [adminEnrollments, setAdminEnrollments] = useState<any>(null);
  const [cleaningUp, setCleaningUp] = useState(false);

  useEffect(() => {
    fetchData();
    checkAdminEnrollments();
  }, []);
  ...
  const checkAdminEnrollments = async () => {
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`${process.env.REACT_APP_API_URL}/admin/enrollments/check`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      if (response.ok) {
        ...
        if (data.success) {
          setAdminEnrollments(data.data);
        }
      }
    } catch (error) {
      console.error('Error checking admin enrollments:', error);
    }
  };

  const cleanupAdminEnrollments = async () => {
    setCleaningUp(true);
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`${process.env.REACT_APP_API_URL}/admin/enrollments/cleanup`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (response.ok) {
        const data = await response.json();
        if (data.success) {
          toast.success(`Removed ${data.data.removedEnrollments} admin enrollments`);
          setAdminEnrollments({ hasEnrollments: false, enrollmentCount: 0, enrollments: [] });
        }
      } else {
        toast.error('Failed to cleanup admin enrollments');
      }
    } catch (error) {
      console.error('Error cleaning up admin enrollments:', error);
      toast.error('Failed to cleanup admin enrollments');
    } finally {
      setCleaningUp(false);
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Admin Enrollment Warning */}
      {adminEnrollments?.hasEnrollments && (
          ...
          <div className="flex items-start">
            ...
            <div className="ml-3 flex-1">
              <h3 className="text-sm font-medium text-yellow-800">
                Admin Account has Course Enrollments
              </h3>
              <div className="mt-2 text-sm text-yellow-700">
                <p>
                  Your admin account has {adminEnrollments.enrollmentCount} course enrollment(s). 
                  Admins should not be enrolled in courses. Would you like to remove these enrollments?
                </p>
              </div>
              <div className="mt-4">
                <button
                  onClick={cleanupAdminEnrollments}
                  disabled={cleaningUp}
                ...
                >
                  {cleaningUp ? 'Removing...' : 'Remove Admin Enrollments'}
                </button>
              </div>
            </div>
          </div>
